...
svm_input_dir="svm_rank/input_files_dib_online" #path to store generated svm input
#input
topic_file="data/topics-with-intruder_dib_online.txt"
intruder_file="data/intruder.txt"
ref_corpus_dir="../../clean_data/ref_corpus"
#output
wordcount_file="wordcount/wc-wi_dib_online.txt"
wi_file="results/topics-wi_dib_online.txt"
...
mkdir $svm_input_dir
python2 GenSVMInput.py $topic_file $intruder_file $pmi_type $wordcount_file > $svm_input_dir/orig.dat

#split the data into ten partitions (for ten-fold cross validations)
echo "Splitting the SVM input ten partitions..."
python2 SplitSVM.py $svm_input_dir < $svm_input_dir/orig.dat
...
python2 ComputeWordIntrusion.py $topic_file $svm_input_dir/test.dat $svm_input_dir/predictions > \
    $wi_file
